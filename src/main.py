...
def cercaBenzinaio():
    ...
    if tipoBenzinaio == 'vicino':
        ...
        query = f"SELECT a.nome, a.indirizzo, a.comune, a.latitudine, a.longitudine, p.TipoCarburante, p.prezzo FROM anagrafica as a JOIN prezzi as p ON a.ID = p.IDImpianto WHERE p.TipoCarburante = '"+ args[3] + "' AND p.isSelf = 1 ORDER BY p.Prezzo ASC ";
        result = db.esegui_query(query)
        ...
        for i in result:
                ...
                if call['features'][0]['properties']['summary']['distance'] < args[5]:
                    validStation.append(result[i])
                    sleep(1.5)
        
        validStation.sort(key=lambda x: x[6])
        
        if validStation:
            bestStation = validStation[0]
            bot.send_message(args[0], "Nome benzinaio: " + str(bestStation[0]) + "\nIndirizzo: " + str(bestStation[1]) + "\nCitta: " + str(bestStation[2]) + "\nPrezzo: " + str(bestStation[6]) + "\nTipo di carburante: " + str(bestStation[5]))
